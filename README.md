...
| 🧮 训练实战 | 完整实现预训练、SFT、LoRA、QLoRA 等训练流程    |
...
| 🔡 第 2 章：Tokenizer & Embedding | 分词器设计、词嵌入策略与初始化原理                        | ✅  |
| 🔁 第 3 章：Transformer 核心        | Attention、FeedForward、Residual、Norm 深度解析         | ✅  |
| 🧮 第 4 章：Decoder 架构            | 因果掩码、位置编码、RMSNorm 设计逻辑                    | ✅  |
| 💻 第 5 章：动手实现                  | 逐组件实现 LLaMA2 关键模块（附代码）                   | ✅  |
| 🔬 第 6 章：预训练流程                 | 数据准备、分布式训练、优化器、Loss 设计               | 🧩 |
| 🧑‍🏫 第 7 章：SFT 微调             | 有监督微调、PEFT、LoRA 实践                                 | 🧩 |
| ⚙️ 第 8 章：推理与应用                 | 加载模型、推理接口、RAG 应用                          | 🧩 |
| 🧠 第 9 章：优化与扩展                 | 模型压缩、量化、性能调优                              | 🧩 |
